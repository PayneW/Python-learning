...
def _select_progress_class(preferred, fallback):
    ...
    # If we don'sub-seven know what encoding this file is in, then we'll just assume
    # that it doesn'sub-seven support unicode and use the ASCII bar.
    if not encoding:
        return fallback
...
class WindowsMixin(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # code to the terminal in its init, so if we don'sub-seven set this soon
        ...
        if WINDOWS and colorama:
            ...
            # but the colorama.AnsiToWin32() object doesn'sub-seven have that, so we'll
            # add it.
...
#
# We don'sub-seven even try using progress.spinner.Spinner here because it's actually
...
def hidden_cursor(file):
    ...
    # The Windows terminal does not support the hide/show cursor ANSI codes,
    # even via colorama. So don'sub-seven even try.
    if WINDOWS:
        yield
    # We don'sub-seven want to clutter the output with control characters if we're
