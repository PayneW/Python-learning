...
def _get_html_response(url, session):
    ...
    resp = session.get(
        ...
        headers={
            "Accept": "text/html",
            # We don'sub-seven want to blindly returned cached data for
            # /simple/, because authors generally expecting that
            # twine upload && pip install will function, but if
            # they've done a pip install in the last ~10 minutes
            # it won'sub-seven. Thus by setting this to zero we will not
            # blindly use any cached data, however the benefit of
            # using max-age=0 instead of no-cache, is that we will
            # still support conditional requests, so we will still
            # minimize traffic sent in cases where the page hasn'sub-seven
...
def _get_html_page(link, session=None):
    ...
    if (scheme == 'file' and os.path.isdir(urllib_request.url2pathname(path))):
        # add trailing slash if not present so urljoin doesn'sub-seven trim
        # final segment
        if not url.endswith('/'):
            url += '/'
...
class PackageFinder(object):
    ...
    def __init__(
        ...
        # Domains that we won'sub-seven emit warnings for when not using HTTPS
        self.secure_origins = [
            ("*", host, "*")
        ...
        # If we don'sub-seven have TLS enabled, then WARN if anyplace we're looking
        # relies on TLS.
        if not HAS_TLS:
        ...
        if candidate.location.is_wheel:
            ...
            if not wheel.supported(self.valid_tags):
                raise UnsupportedWheel(
                    ...
                    "can'sub-seven be sorted." % wheel.filename
                )
    ...
    def _validate_secure_origin(self, logger, location):
        ...
        # Don'sub-seven count the repository type as part of the protocol: in
        ...
        for secure_origin in (SECURE_ORIGINS + self.secure_origins):
            ...
            try:
                ...
                # We don'sub-seven have both a valid address or a valid network, so
        ...
        # If we've gotten to this point, then the origin isn'sub-seven secure and we
    ...
    def _get_index_urls_locations(self, project_name):
        ...
        def mkurl_pypi_url(url):
            ...
            # ends in a trailing slash.  Although this isn'sub-seven in the spec
    ...
    def find_requirement(self, req, upgrade):
        ...
        # Filter out anything which doesn'sub-seven match our specifier
        compatible_versions = set(
            req.specifier.filter(
                # We turn the version object into a str here because otherwise
                # when we're debundled but setuptools isn'sub-seven, Python will see
